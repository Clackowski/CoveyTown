...
import ShopAreaFactory from './shops/ShopAreaFactory';
import TradeAreaFactory from './trading/TradeAreaFactory';
...
export default class Town {
  ...
  public initializeFromMap(map: ITiledMap) {
    ...
    const shopAreas = objectLayer.objects
      ...
      .map(eachShopAreaObj => ShopAreaFactory(eachShopAreaObj, this._broadcastEmitter));
    const tradeAreas = objectLayer.objects
      .filter(eachObject => eachObject.type === 'TradeArea')
      .map(eachTradeAreaObj => TradeAreaFactory(eachTradeAreaObj, this._broadcastEmitter));
