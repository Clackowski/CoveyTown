...
import ShopAreaController, { ShopEventTypes } from './interactable/ShopAreaController';
import HatAreaController from './interactable/HatAreaController';
import TradingAreaController, { TradeEventTypes } from './interactable/TradingAreaController';
import TradeArea from '../components/Town/interactables/TradeArea';
import HatTradingAreaController from './interactable/HatTradingAreaController';
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  public get shopAreas() {
    ...
    return ret as ShopAreaController<ShopState, ShopEventTypes>[];
  }

  public get tradeAreas() {
    const ret = this._interactableControllers.filter(
      eachInteractable => eachInteractable instanceof TradingAreaController,
    );
    return ret as TradingAreaController<TradeState, TradeEventTypes>[];
  }
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this._socket.on('initialize', initialData => {
        ...
        initialData.interactables.forEach(eachInteractable => {
          ...
          } else if (isHatShopArea(eachInteractable)) {
            this._interactableControllers.push(
              new HatAreaController(eachInteractable.id, eachInteractable, this),
            );
          } else if (isHatTradingArea(eachInteractable)) {
            this._interactableControllers.push(
              new HatTradingAreaController(eachInteractable.id, eachInteractable, this),
            );
          }
        });
  ...
  public getShopAreaController<ShopType extends ShopState, EventsType extends ShopEventTypes>(
    shopArea: ShopArea,
  ): ShopAreaController<ShopType, EventsType> {
    const existingController = this._interactableControllers.find(
      eachExistingArea => eachExistingArea.id === shopArea.name,
    );
    if (existingController instanceof ShopAreaController) {
      return existingController as ShopAreaController<ShopType, EventsType>;
    } else {
      throw new Error('Shop area controller not created');
    }
  }

  public getTradingAreaController<TradeType extends TradeState, EventsType extends TradeEventTypes>(
    tradeArea: TradeArea,
  ): TradingAreaController<TradeType, EventsType> {
    const existingController = this._interactableControllers.find(
      eachExistingArea => eachExistingArea.id === tradeArea.name,
    );
    if (existingController instanceof TradingAreaController) {
      return existingController as TradingAreaController<TradeType, EventsType>;
    } else {
      throw new Error('Trading area controller not created');
    }
  }
...
export function useInteractableShopAreaController<T>(interactableAreaID: string): T {
  const townController = useTownController();
  const interactableAreaController = townController.shopAreas.find(
    eachArea => eachArea.id == interactableAreaID,
  );
  if (!interactableAreaController) {
    throw new Error(`Requested interactable area ${interactableAreaID} does not exist`);
...
export function useInteractableTradeAreaController<T>(interactableAreaID: string): T {
  const townController = useTownController();
  const interactableAreaController = townController.tradeAreas.find(
    eachArea => eachArea.id == interactableAreaID,
  );
  if (!interactableAreaController) {
    throw new Error(`Requested interactable area ${interactableAreaID} does not exist`);
