...
import { Player as PlayerModel, PlayerLocation, HatType } from '../types/CoveyTownSocket';
...
export default class PlayerController extends (EventEmitter as new () => TypedEmitter<PlayerEvents>) {
  ...
  public inventory: Map<HatType, number>;

  public currentHat: HatType;

  public coveycoins: number;
  ...
  async updateLocalInventory(): Promise<void> {
    await this._playerData.addPlayer(this._userName);
    this.inventory = new Map();
    ...
    for (let hatIndex = 0; hatIndex < hats.length; hatIndex++) {
      this.inventory.set(hats[hatIndex] as HatType, await this._playerData.getHatQuantity(this.userName, hats[hatIndex]));
    }
    const currHat = await this._playerData.getActiveHat(this.userName);
    this.currentHat = currHat;

    const coins = await this._playerData.getCoinCount(this.userName);
    this.coveycoins = coins;
  }

  async updateCurrentHat(activeHat: HatType): Promise<void> {
    this.currentHat = activeHat;
    await this._playerData.setActiveHat(this.userName, activeHat);
  }
